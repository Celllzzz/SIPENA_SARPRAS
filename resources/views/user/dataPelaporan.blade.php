        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">
                    Daftar seluruh laporan kerusakan sarana dan prasarana yang diajukan
                </p>
            </div>
            <a href="{{ route('pelaporan.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-teal-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-wider hover:bg-teal-700 transition shadow-sm">
                Buat Laporan Baru
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-6 sm:py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-md shadow-sm border border-gray-200 p-5 sm:p-6"
                ...
                {{-- Toolbar Filter & Gabungan Input Search --}}
                <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 mb-5">
                    <form method="GET" action="{{ route('pelaporan.index') }}" class="flex items-center text-xs text-gray-600">
                    ...
                    {{-- Search Input Digabungkan dengan Tombol (Teal) --}}
                ...
                {{-- Skeleton Placeholder saat Memuat Data --}}
                            ...
                            @forelse ($pelaporans as $laporan)
                                <tr class="hover:bg-gray-50/70 transition-colors">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-xs sm:text-sm font-medium text-center">
                                        <a href="{{ route('pelaporan.show', $laporan->id) }}" class="inline-flex items-center px-3 py-1 rounded-md text-xs font-semibold bg-teal-50 text-teal-700 hover:bg-teal-100 transition-colors">
                                            Rincian &rarr;
                                        </a>
                                    </td>
                                </tr>
